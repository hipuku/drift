...
The overview leads with a sentence, not a table. Where the system has drifted, in plain terms,
followed by a card per category tinted by verdict — red where something needs review, green
where it holds. The colour *is* the finding, so the shape of the page reads before any number
does.
...
The pairing is the part that matters. Contrast is measured against the background a reader
actually sees — the nearest non-transparent ancestor, with alpha composited — not against
whatever the element itself declares. Muted text at 50% opacity on a tinted panel is judged as
it renders, which is usually the case a naive check gets wrong: evaluated as authored, 50%
black on white measures 18.9 and passes AAA; composited, it measures 3.5 and fails AA.
...
The ruler is the evidence. Sizes are plotted on a log axis against a modular scale's steps,
and the ones that miss a step sit between ticks in red. The scale is **selectable** — every
named ratio is offered with its own off-count, so the strip answers "which scale is this
system actually on?" rather than only telling you how far you are from one you didn't choose.
...
someone typed rather than chose. Border widths use a finer 0.5px threshold, because 1px versus
1.5px is a real duplicate at that scale.
...
The categories most audits skip, because they are where drift hides rather than where it
shows.

![Opacity](screenshots/opacity.png)
![Z-index](screenshots/z-index.png)

Z-index is rendered as a stacking ladder rather than a number, so an ad-hoc set is visible as
a shape. A set larger than eight layers, or one containing a 9999, is called out on the health
line: both are the signature of values picked to win rather than to sit in a scale.
...
The whole audit as one JSON artefact, leading with the diagnosis — `health`, then `findings[]`
with severity and evidence, then `verdicts`, then the `rules` each number was measured
against — and carrying the full inventory underneath.

It is built for machines rather than for reading: assert on it in CI, diff two runs to see
what moved, or hand it to a model and ask what to fix first. The rules block is what makes
that possible, since a count is meaningless without the reference it was counted against.
